...
func TestGetOrCreateConnector_DifferentIDs(t *testing.T) {
	...
	set1 := connectortest.NewNopSettings()
	set2 := connectortest.NewNopSettings()
...
func TestCreateTracesToTraces(t *testing.T) {
	...
	conn, err := factory.CreateTracesToTraces(context.Background(), set, cfg, consumertest.NewNop())
...
func TestCreateTracesToLogs(t *testing.T) {
	...
	conn, err := factory.CreateTracesToLogs(context.Background(), set, cfg, consumertest.NewNop())
	require.NoError(t, err)
	assert.NotNil(t, conn)
...
func TestStartShutdown(t *testing.T) {
	...
	assert.NoError(t, c.Start(context.Background(), componenttest.NewNopHost()))
...
func TestConsumeTraces_OISpanTransformed(t *testing.T) {
	...
	require.Equal(t, 1, len(tracesSink.AllTraces()))
...
func TestConsumeTraces_LLOEmitsLogs(t *testing.T) {
	...
	require.Equal(t, 1, len(logsSink.AllLogs()))
	assert.Greater(t, logsSink.AllLogs()[0].LogRecordCount(), 0)
...
func TestLogsOnlyConnector_ConsumeTracesNoOp(t *testing.T) {
	...
	err := wrapper.ConsumeTraces(context.Background(), td)
